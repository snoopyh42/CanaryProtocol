...
class ABTestRunner:
    ...
    def add_test_case(self, headlines, economic_data, urgency_level, expected_characteristics=None):
    ...
    def evaluate_response(self, response_text, expected_characteristics):
        ...
        if 800 <= word_count <= 1200:  # Target range
            ...
            feedback.append(f"⚠️ Length: {word_count} words (target: 800-1200)")
        ...
        sections_found = sum(1 for section in required_sections if section in response_text)
        score += (sections_found / len(required_sections)) * 20
        feedback.append(f"📋 Sections: {sections_found}/{len(required_sections)}")
        ...
        import re
        numbers = len(re.findall(r'\d+(?:\.\d+)?%|\$[\d,]+|\d+(?:,\d+)*', response_text))
        if numbers >= 5:
            score += 15
        ...
        status_symbols = response_text.count("🟢") + response_text.count("🟠") + response_text.count("🔴")
        ...
        problematic_terms = ["collapse", "chaos", "catastrophic", "civil war", "apocalypse"]
        problematic_found = sum(1 for term in problematic_terms if term.lower() in response_text.lower())
    ...
    def run_full_test(self):
        ...
        for test_idx, test_case in enumerate(self.test_data):
            ...
            headlines_text = "\n".join([f"{h['title']} - {h['url']}" for h in test_case["headlines"]])
            economic_context = ""
            if test_case["economic_data"]:
                high_concern = [e for e in test_case["economic_data"] if e.get('concern_level') == 'high']
                if high_concern:
...
{headlines_text}"""
            ...
            for variant_name in self.variants:
                ...
                if "error" not in result:
                    evaluation = self.evaluate_response(result["response"], test_case.get("expected", {}))
    ...
    def generate_report(self, results_summary):
        ...
        for variant_name, evaluations in results_summary.items():
            ...
            # Aggregate metrics
            avg_word_count = sum(e["word_count"] for e in evaluations) / len(evaluations)
            avg_sections = sum(e["sections_found"] for e in evaluations) / len(evaluations)
            avg_links = sum(e["link_count"] for e in evaluations) / len(evaluations)
        ...
        # Determine winner
        best_variant = max(results_summary.keys(), 
                          key=lambda v: sum(e["score"] for e in results_summary[v]) / len(results_summary[v]))
    ...
    return [
        {
            ...
            "expected": {"min_word_count": 800, "sections": 5}
        }
    ]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        choice = input("Run: (1) Quick test, (2) Full analysis, (3) Setup only: ")
    ...
    if choice in ["1", "2"]:
        if not os.getenv("OPENAI_API_KEY"):
            ...
            print("⚠️  A/B testing requires OpenAI API calls and may use significant credits")
            print("🧪 This is a demonstration framework - customize test cases for production use")
