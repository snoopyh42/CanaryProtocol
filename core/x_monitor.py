...
class XMonitor:
    ...
    def __init__(self, db_path="data/canary_protocol.db", config_path="config/.env"):
    ...
    def get_weekly_trends(self):
        ...
        if self.config_enabled and self.social_config.get('enabled', True):
            political_keywords = self.social_config.get('political_keywords', [
                "Trump", "Biden", "Congress", "Supreme Court", "protest", "voting"
            ])
            economic_keywords = self.social_config.get('economic_keywords', [
                "economy", "inflation", "recession", "stock market", "Fed", "unemployment"
            ])
            max_keywords = self.social_config.get('max_keywords_per_request', 8)
        else:
            # Fallback defaults
            political_keywords = ["Trump", "Biden", "Congress", "Supreme Court", "protest", "voting"]
            economic_keywords = ["economy", "inflation", "recession", "stock market", "Fed", "unemployment"]
            max_keywords = 8
        ...
        political_trends = self._analyze_keyword_trends(political_keywords, start_date, end_date, "political")
        
        # Only proceed with economic if we have API calls left
        economic_trends = {}
        if political_trends:  # If we got political data successfully
            economic_trends = self._analyze_keyword_trends(economic_keywords, start_date, end_date, "economic")
        
        # Monitor notable accounts if configured and we have API calls remaining
        account_activity = {}
        if self.config_enabled and political_trends and economic_trends:
            account_activity = self._monitor_notable_accounts(start_date, end_date)
        
        # Store results
        self._store_weekly_analysis(political_trends, economic_trends, account_activity)
    ...
    def _analyze_keyword_trends(self, keywords, start_date, end_date, category):
        ...
        for i, keyword in enumerate(limited_keywords):
            try:
                ...
                base_delay = self.social_config.get('request_delay_seconds', 5) if self.config_enabled else 5
                delay = base_delay + (i * 2)  # Increase delay for each request
                
                print(f"   ⏳ Waiting {delay}s before analyzing '{keyword}'...")
                time.sleep(delay)
                
                trend_data = self._search_keyword_week(keyword, start_date, end_date)
                if trend_data:
                    ...
                    if i == 0:  # If first keyword fails, abort category
                        print(f"   🛑 Aborting {category} analysis - likely rate limited")
                        break
    ...
    def _monitor_notable_accounts(self, start_date, end_date):
        ...
        for i, (username, category) in enumerate(all_accounts[:3]):  # Max 3 total
            try:
                print(f"   Checking @{username} ({category})...")
                time.sleep(delay)
                
                activity = self._get_account_activity(username, start_date, end_date)
                if activity:
                    account_activity[username] = {**activity, 'category': category}
                    print(f"   ✅ @{username}: {activity.get('tweet_count', 0)} tweets")
                else:
    ...
    def _search_keyword_week(self, keyword, start_date, end_date):
        ...
        # Build search query - use recent search endpoint instead of full archive
    ...
    def _process_tweet_data(self, data, keyword):
        ...
        engagement_score = (total_likes + total_retweets * 2 + total_replies) / max(total_tweets, 1)
        
        # Sentiment analysis (basic - could be enhanced)
        sentiment_keywords = {
            'positive': ['good', 'great', 'excellent', 'amazing', 'wonderful', 'fantastic'],
            'negative': ['bad', 'terrible', 'awful', 'horrible', 'disaster', 'crisis', 'fail']
        }
        ...
        sentiment_score = (positive_count - negative_count) / max(total_tweets, 1)
    ...
    def _calculate_trending_velocity(self, tweets):
        ...
        if len(recent_days) == 2:
            velocity = daily_counts[recent_days[1]] - daily_counts[recent_days[0]]
            return velocity / max(daily_counts[recent_days[0]], 1)
        
        return 0
    
    def _store_weekly_analysis(self, political_trends, economic_trends, account_activity=None):
    ...
    def _extract_viral_content(self, political_trends, economic_trends, account_activity=None):
        """Extract most viral content indicators"""
        viral_indicators = []
        
        # Check political trends for viral content
        for keyword, data in political_trends.items():
            if data.get('engagement_score', 0) > 50:  # High engagement threshold
        ...
        if account_activity:
            for username, data in account_activity.items():
                if data.get('engagement_score', 0) > 75:  # Higher threshold for accounts
                    viral_indicators.append(f"@{username}: {data['engagement_score']} engagement")
        
        return '; '.join(viral_indicators[:5])  # Top 5 viral indicators
    
    def _calculate_engagement_metrics(self, political_trends, economic_trends, account_activity=None):
    ...
    def _calculate_urgency_score(self, political_trends, economic_trends, account_activity=None):
        ...
        max_boost = self.social_config.get('urgency_thresholds', {}).get('max_urgency_boost', 5) if self.config_enabled else 5
    ...
    def generate_social_media_summary(self):
        ...
        try:
            ...
            if political_trends:
                pol_summary = self._summarize_trend_category(political_trends, "Political")
                summary_parts.append(pol_summary)
            
            # Economic trends summary  
            if economic_trends:
                econ_summary = self._summarize_trend_category(economic_trends, "Economic")
    ...
    def _summarize_trend_category(self, trends, category):
        ...
        sorted_trends = sorted(trends.items(), key=lambda x: x[1].get('engagement_score', 0), reverse=True)
